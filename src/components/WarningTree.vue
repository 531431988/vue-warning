<template>
  <a-spin :spinning="spinning">
    <div class="warning-tree">
      <div class="warning-tree-hd">
        <slot name="select"></slot>
        <a-input v-model="searchValue" placeholder="请输入指标名称" @change="onChange" v-if="search">
          <i class="icon-ic-search" slot="suffix"></i>
        </a-input>
      </div>
...
const TreeNode = {
  ...
  render () {
    ...
    else if (state === '重度预警') color = 'color: #FF5B5B'
...
export default {
  ...
  watch: {
    data: {
      handler (newVal) {
        this.treeData = this.generateData(newVal)
        this.getValKeyList(newVal)
        this.spinning = false
      },
      deep: true
    }
...
.warning-tree {
  ...
  &-scroll {
    padding: 0 16px;
    height: calc(100vh - 44px - 60px - 60px);
    overflow-y: auto;
    overflow-x: hidden;
  }
